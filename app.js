(() => {
  ...
  const els = {
    ...
    personReport: $('#personReport'), categoryReport: $('#categoryReport'), necessaryChart: $('#necessaryChart'), dailyChart: $('#dailyChart'), incomeExpenseChart: $('#incomeExpenseChart'), savingsChart: $('#savingsChart'), weeklyChart: $('#weeklyChart'), monthCompareChart: $('#monthCompareChart'), analyticsSummary: $('#analyticsSummary'), topExpensesReport: $('#topExpensesReport'), topDaysReport: $('#topDaysReport'), exportMonthlyReportBtn: $('#exportMonthlyReportBtn'), incomeReport: $('#incomeReport'), accountReport: $('#accountReport'),
    calendarGrid: $('#calendarGrid'), dayDetails: $('#dayDetails'),
    notesLockBox: $('#notesLockBox'), notesPinInput: $('#notesPinInput'), notesUnlockBtn: $('#notesUnlockBtn'), notesSetPinBtn: $('#notesSetPinBtn'), notesError: $('#notesError'), notesArea: $('#notesArea'), notesSearch: $('#notesSearch'), notesFilter: $('#notesFilter'), notesSort: $('#notesSort'), notesList: $('#notesList'), addNoteCardBtn: $('#addNoteCardBtn'), notesLockBtn: $('#notesLockBtn'),
    settingExpectedIncome: $('#settingExpectedIncome'), settingSavingTarget: $('#settingSavingTarget'), settingNecessaryLimit: $('#settingNecessaryLimit'), settingUnnecessaryLimit: $('#settingUnnecessaryLimit'), saveBudgetBtn: $('#saveBudgetBtn'), settingsPeopleList: $('#settingsPeopleList'), settingsCategoryList: $('#settingsCategoryList'), settingsAccountList: $('#settingsAccountList'), settingsPersonInput: $('#settingsPersonInput'), settingsCategoryInput: $('#settingsCategoryInput'), settingsAccountInput: $('#settingsAccountInput'), settingsAddPerson: $('#settingsAddPerson'), settingsAddCategory: $('#settingsAddCategory'), settingsAddAccount: $('#settingsAddAccount'), syncUrl: $('#syncUrl'), saveSyncUrlBtn: $('#saveSyncUrlBtn'), syncNowBtn: $('#syncNowBtn'), importSheetBtn: $('#importSheetBtn'), syncMessage: $('#syncMessage'), exportJsonBtn: $('#exportJsonBtn'), exportCsvBtn: $('#exportCsvBtn'), importFile: $('#importFile'), resetBtn: $('#resetBtn'), appPinInput: $('#appPinInput'), notesPinSettingInput: $('#notesPinSettingInput'), savePinsBtn: $('#savePinsBtn'), createArchiveBtn: $('#createArchiveBtn'), archiveList: $('#archiveList'), activityLogList: $('#activityLogList')
  };
  ...
  function normalize(st){
    ...
    st.notes = (st.notes||[]).map(e=>({...e,id:e.id||uid('note'),type:'note',date:e.date||e.createdAt?.slice(0,10)||todayISO(),title:e.title||'Note',description:e.description||'',tag:e.tag||'',priority:e.priority||'normal',dueDate:e.dueDate||'',pinned:!!e.pinned,favorite:!!e.favorite,archived:!!e.archived,createdAt:e.createdAt||new Date().toISOString(),updatedAt:e.updatedAt||new Date().toISOString()}));
  ...
  function clearForm(){ els.editingId.value=''; els.editingType.value=''; els.entryDate.value=todayISO(); els.entryAmount.value=''; els.entryTitle.value=''; els.entryDescription.value=''; selectedPeople=[]; selectedCategories=[]; els.entryNecessary.checked=true; els.noteTag.value=''; if(els.notePriority) els.notePriority.value='normal'; if(els.noteDueDate) els.noteDueDate.value=''; els.debtStatus.value='receivable'; renderChips(); setAddType(els.entryType.value||'expense', false); }
  function renderAll(){ renderHome(); renderChips(); renderEntries(); renderReports(); renderCalendar(); renderNotesLock(); renderSettings(); renderActivityLog(); renderArchives(); }
  ...
  function saveEntry(e){ e.preventDefault(); const type=els.entryType.value; const id=els.editingId.value || uid(type); const base={id,type,date:els.entryDate.value||todayISO(),title:els.entryTitle.value.trim()||type[0].toUpperCase()+type.slice(1),description:els.entryDescription.value.trim(),updatedAt:new Date().toISOString(),createdAt:new Date().toISOString()}; const amount=Number(els.entryAmount.value)||0; let obj={...base};
    ...
    if(type==='note'){ obj={...obj,tag:els.noteTag.value.trim(),priority:els.notePriority?.value||'normal',dueDate:els.noteDueDate?.value||'',pinned:false,favorite:false,archived:false}; const existing=state.notes.find(n=>n.id===id); if(existing) obj={...existing,...obj,pinned:!!existing.pinned,favorite:!!existing.favorite,archived:!!existing.archived}; upsert('notes',obj); }
  ...
  function renderReports(){
    const t=totals();
    const prev=monthTotals(addMonths(state.settings.selectedMonth,-1));
    const byPeople={}; t.expenses.forEach(e=>(e.peopleIds||['combined-general']).forEach(id=>byPeople[id]=(byPeople[id]||0)+Number(e.amount||0)));
    const byCat={}; t.expenses.forEach(e=>(e.categoryIds||['other']).forEach(id=>byCat[id]=(byCat[id]||0)+Number(e.amount||0)));
    const byInc={}; t.incomes.forEach(i=>byInc[i.source||'Other Income']=(byInc[i.source||'Other Income']||0)+Number(i.amount||0));
    const daily=dailyTotals(t.expenses);
    const weekly=weeklyTotals(t.expenses);
    const topExpense=t.expenses.slice().sort((a,b)=>Number(b.amount||0)-Number(a.amount||0))[0];
    const topDay=Object.entries(dailyByDate(t.expenses)).sort((a,b)=>b[1]-a[1])[0];
    const topPerson=Object.entries(byPeople).sort((a,b)=>b[1]-a[1])[0];
    const topCat=Object.entries(byCat).sort((a,b)=>b[1]-a[1])[0];
    if(els.analyticsSummary){
      els.analyticsSummary.innerHTML = [
        ['Monthly saving', money(t.bal), 'Income minus expense for the selected month.'],
        ['Daily average', money(t.exp ? t.exp / Math.max(1,new Date(Number(state.settings.selectedMonth.slice(0,4)),Number(state.settings.selectedMonth.slice(5,7)),0).getDate()) : 0), 'Average spending per calendar day.'],
        ['Top person', topPerson ? `${nameById(state.people,topPerson[0])} · ${money(topPerson[1])}` : 'No data', 'Family member with the highest spending.'],
        ['Top category', topCat ? `${nameById(state.categories,topCat[0])} · ${money(topCat[1])}` : 'No data', 'Category where most money was spent.'],
        ['Highest day', topDay ? `${topDay[0]} · ${money(topDay[1])}` : 'No data', 'Date with the highest total spending.'],
        ['Largest expense', topExpense ? `${topExpense.title} · ${money(topExpense.amount)}` : 'No data', 'Single biggest expense entry.']
      ].map(([a,b,c])=>`<article class="mini-card analytics-card"><span class="muted">${esc(a)}</span><strong>${esc(b)}</strong><small class="muted">${esc(c)}</small></article>`).join('');
    }
    rank(els.personReport, byPeople, state.people, 'person');
    rank(els.categoryReport, byCat, state.categories, 'category');
    rankRaw(els.incomeReport, byInc);
    const bals=accountBalances(); rankRaw(els.accountReport, Object.fromEntries(state.accounts.map(a=>[a.name,bals[a.id]||0])));
    rankTopExpenses(els.topExpensesReport, t.expenses);
    rankTopDays(els.topDaysReport, dailyByDate(t.expenses));
    drawPie(els.necessaryChart,[['Necessary',t.nec],['Unnecessary',t.un]]);
    drawLine(els.dailyChart,daily);
    if(els.incomeExpenseChart) drawBars(els.incomeExpenseChart,[['Income',t.inc],['Expense',t.exp],['Balance',Math.max(0,t.bal)]]);
    if(els.savingsChart) drawBars(els.savingsChart,[['Saved',Math.max(0,t.bal)],['Target',Number(state.settings.budget.savingTarget)||0]]);
    if(els.weeklyChart) drawBars(els.weeklyChart, weekly.map((v,i)=>[`Week ${i+1}`,v]));
    if(els.monthCompareChart) drawGroupedBars(els.monthCompareChart,[['Income',prev.inc,t.inc],['Expense',prev.exp,t.exp],['Savings',prev.bal,t.bal]]);
  }
  function monthTotals(mk){ const expenses=state.expenses.filter(x=>(x.date||'').startsWith(mk)), incomes=state.incomes.filter(x=>(x.date||'').startsWith(mk)); const inc=sum(incomes), exp=sum(expenses), nec=sum(expenses.filter(x=>x.necessary!==false)), un=exp-nec; return {expenses,incomes,inc,exp,nec,un,bal:inc-exp}; }
  function addMonths(mk,n){ const d=new Date(`${mk}-01T12:00:00`); d.setMonth(d.getMonth()+n); return `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}`; }
  function dailyByDate(expenses){ const out={}; expenses.forEach(e=>{ out[e.date]=(out[e.date]||0)+Number(e.amount||0); }); return out; }
  function weeklyTotals(expenses){ const weeks=[0,0,0,0,0,0]; expenses.forEach(e=>{ const d=Number((e.date||'').slice(8,10)); if(d) weeks[Math.floor((d-1)/7)] += Number(e.amount)||0; }); return weeks.filter((_,i)=>i<5 || weeks[i]); }
  function rankTopExpenses(root,expenses){ if(!root) return; const list=expenses.slice().sort((a,b)=>Number(b.amount||0)-Number(a.amount||0)).slice(0,10); const max=Math.max(1,...list.map(x=>Number(x.amount)||0)); root.innerHTML=list.length?list.map(x=>`<div class="rank-item"><strong>${esc(x.title||'Expense')} <small class="muted">${esc(x.date)}</small></strong><span>${money(x.amount)}</span><div class="bar-line"><span style="width:${Math.round((Number(x.amount)||0)/max*100)}%"></span></div></div>`).join(''):`<p class="muted">No expense data yet.</p>`; }
  function rankTopDays(root,byDay){ if(!root) return; const items=Object.entries(byDay).sort((a,b)=>b[1]-a[1]).slice(0,10); const max=Math.max(1,...items.map(x=>x[1])); root.innerHTML=items.length?items.map(([d,v])=>`<div class="rank-item"><strong>${esc(new Date(d+'T12:00:00').toLocaleDateString('en-PK',{day:'numeric',month:'short',year:'numeric'}))}</strong><span>${money(v)}</span><div class="bar-line"><span style="width:${Math.round(v/max*100)}%"></span></div></div>`).join(''):`<p class="muted">No daily spending data yet.</p>`; }
  ...
  function chartBase(canvas){ if(!canvas) return null; const ctx=canvas.getContext('2d'); const w=canvas.width=Math.max(280,canvas.clientWidth)*devicePixelRatio, h=canvas.height=Math.max(150,canvas.clientHeight)*devicePixelRatio; ctx.clearRect(0,0,w,h); ctx.font=`${12*devicePixelRatio}px sans-serif`; return {ctx,w,h,p:24*devicePixelRatio}; }
  function drawPie(canvas,data){ const base=chartBase(canvas); if(!base) return; const {ctx,w,h}=base; const total=data.reduce((a,b)=>a+b[1],0)||1; let start=-Math.PI/2; const colors=['#174f3f','#d39a38','#8a4b2d']; data.forEach((d,i)=>{ const ang=d[1]/total*Math.PI*2; ctx.beginPath(); ctx.moveTo(w/2,h/2); ctx.arc(w/2,h/2,Math.min(w,h)/3,start,start+ang); ctx.closePath(); ctx.fillStyle=colors[i%colors.length]; ctx.fill(); start+=ang; }); ctx.fillStyle='#13261f'; ctx.fillText(`Necessary ${Math.round(data[0][1]/total*100)}%`,12*devicePixelRatio,22*devicePixelRatio); }
  function drawLine(canvas,data){ const base=chartBase(canvas); if(!base) return; const {ctx,w,h,p}=base; const max=Math.max(1,...data); ctx.strokeStyle='#e7e2d6'; ctx.lineWidth=1*devicePixelRatio; for(let i=0;i<4;i++){ const y=p+i*(h-2*p)/3; ctx.beginPath(); ctx.moveTo(p,y); ctx.lineTo(w-p,y); ctx.stroke(); } ctx.strokeStyle='#174f3f'; ctx.lineWidth=3*devicePixelRatio; ctx.beginPath(); data.forEach((v,i)=>{ const x=p+i*(w-2*p)/(data.length-1||1); const y=h-p-(v/max)*(h-2*p); i?ctx.lineTo(x,y):ctx.moveTo(x,y); }); ctx.stroke(); }
  function drawBars(canvas,data){ const base=chartBase(canvas); if(!base) return; const {ctx,w,h,p}=base; const max=Math.max(1,...data.map(x=>Math.abs(x[1]))); const gap=12*devicePixelRatio; const bw=(w-2*p-gap*(data.length-1))/Math.max(1,data.length); data.forEach(([label,val],i)=>{ const x=p+i*(bw+gap); const bh=Math.max(2,Math.abs(val)/max*(h-3*p)); ctx.fillStyle='#174f3f'; ctx.fillRect(x,h-p-bh,bw,bh); ctx.fillStyle='#13261f'; ctx.fillText(String(label).slice(0,10),x,h-6*devicePixelRatio); }); }
  function drawGroupedBars(canvas,data){ const base=chartBase(canvas); if(!base) return; const {ctx,w,h,p}=base; const max=Math.max(1,...data.flatMap(x=>[Math.abs(x[1]),Math.abs(x[2])])); const groupW=(w-2*p)/data.length; data.forEach(([label,prev,cur],i)=>{ const x=p+i*groupW+8*devicePixelRatio; const bw=groupW/3; const b1=Math.abs(prev)/max*(h-3*p), b2=Math.abs(cur)/max*(h-3*p); ctx.fillStyle='#d39a38'; ctx.fillRect(x,h-p-b1,bw,b1); ctx.fillStyle='#174f3f'; ctx.fillRect(x+bw+4*devicePixelRatio,h-p-b2,bw,b2); ctx.fillStyle='#13261f'; ctx.fillText(String(label).slice(0,9),x,h-6*devicePixelRatio); }); }
  function exportMonthlyReport(){ const t=totals(); const byPeople={}; t.expenses.forEach(e=>(e.peopleIds||[]).forEach(id=>byPeople[nameById(state.people,id)]=(byPeople[nameById(state.people,id)]||0)+Number(e.amount||0))); const byCat={}; t.expenses.forEach(e=>(e.categoryIds||[]).forEach(id=>byCat[nameById(state.categories,id)]=(byCat[nameById(state.categories,id)]||0)+Number(e.amount||0))); const lines=[`Family Expense Tracker Monthly Report`,`${fmtMonth(state.settings.selectedMonth)}`,``, `Total income,${t.inc}`, `Total expense,${t.exp}`, `Remaining balance,${t.bal}`, `Necessary expense,${t.nec}`, `Unnecessary expense,${t.un}`, ``, `Person-wise spending`, ...Object.entries(byPeople).sort((a,b)=>b[1]-a[1]).map(([k,v])=>`${k},${v}`), ``, `Category-wise spending`, ...Object.entries(byCat).sort((a,b)=>b[1]-a[1]).map(([k,v])=>`${k},${v}`)]; download(`family-monthly-report-${state.settings.selectedMonth}.csv`, lines.join('\n'), 'text/csv'); }
  function renderCalendar(){ const mk=state.settings.selectedMonth, y=+mk.slice(0,4), m=+mk.slice(5,7)-1; const first=new Date(y,m,1), days=new Date(y,m+1,0).getDate(); const byDate={}; monthFilter(state.expenses).forEach(e=>{ byDate[e.date]=byDate[e.date]||{expense:0,income:0,notes:0,tags:new Set()}; byDate[e.date].expense+=Number(e.amount)||0; (e.peopleIds||[]).slice(0,2).forEach(id=>byDate[e.date].tags.add(nameById(state.people,id).split(' ')[0])); }); monthFilter(state.incomes).forEach(e=>{ byDate[e.date]=byDate[e.date]||{expense:0,income:0,notes:0,tags:new Set()}; byDate[e.date].income+=Number(e.amount)||0; byDate[e.date].tags.add('Income'); }); monthFilter(state.notes).filter(n=>!n.archived).forEach(n=>{ byDate[n.date]=byDate[n.date]||{expense:0,income:0,notes:0,tags:new Set()}; byDate[n.date].notes++; byDate[n.date].tags.add('Note'); }); let html=''; const mondayOffset=(first.getDay()+6)%7; for(let i=0;i<mondayOffset;i++) html+='<div class="cal-cell blank"></div>'; for(let d=1;d<=days;d++){ const date=`${mk}-${String(d).padStart(2,'0')}`, x=byDate[date]; html+=`<button class="cal-cell" data-date="${date}"><strong>${d}</strong>${x?`<div class="cal-dots">${x.income?'<i class="dot income"></i>':''}${x.expense?'<i class="dot expense"></i>':''}${x.notes?'<i class="dot note"></i>':''}</div>${x.expense?`<div class="total">${money(x.expense)}</div>`:''}<div class="tags">${[...x.tags].slice(0,3).map(t=>`<span>${esc(t)}</span>`).join('')}</div>`:''}</button>`; } els.calendarGrid.innerHTML=html; els.calendarGrid.onclick=e=>{ const c=e.target.closest('[data-date]'); if(c) dayDetails(c.dataset.date); }; }
  ...
  function renderNotes(){ let list=state.notes.slice(); const filter=els.notesFilter?.value||'active'; if(filter==='active') list=list.filter(n=>!n.archived); if(filter==='pinned') list=list.filter(n=>n.pinned&&!n.archived); if(filter==='favorites') list=list.filter(n=>n.favorite&&!n.archived); if(filter==='archive') list=list.filter(n=>n.archived); const q=(els.notesSearch.value||'').toLowerCase(); if(q) list=list.filter(n=>[n.title,n.description,n.tag,n.priority,n.dueDate].join(' ').toLowerCase().includes(q)); const priorityRank={high:3,normal:2,low:1}; const sort=els.notesSort?.value||'latest'; list.sort((a,b)=> (Number(b.pinned)-Number(a.pinned)) || (sort==='oldest'?String(a.updatedAt).localeCompare(String(b.updatedAt)):sort==='priority'?(priorityRank[b.priority||'normal']-priorityRank[a.priority||'normal']):String(b.updatedAt).localeCompare(String(a.updatedAt)))); els.notesList.innerHTML=list.length?list.map(n=>`<article class="note-card ${n.pinned?'pinned':''}"><div class="note-top"><h3>${n.pinned?'📌 ':''}${esc(n.title)}</h3><span class="tag priority-${esc(n.priority||'normal')}">${esc(n.priority||'normal')}</span></div><p class="muted">${esc(n.description||'No description')}</p><div class="tag-row">${n.tag?`<span class="tag">${esc(n.tag)}</span>`:''}${n.dueDate?`<span class="tag">Due ${esc(n.dueDate)}</span>`:''}${n.favorite?'<span class="tag">Favorite</span>':''}${n.archived?'<span class="tag">Archived</span>':''}<span class="tag">${esc(n.date)}</span></div><div class="actions"><button class="btn tiny soft" data-note-action="pin" data-id="${n.id}">${n.pinned?'Unpin':'Pin'}</button><button class="btn tiny soft" data-note-action="favorite" data-id="${n.id}">${n.favorite?'Unfavorite':'Favorite'}</button><button class="btn tiny soft" data-note-action="archive" data-id="${n.id}">${n.archived?'Restore':'Archive'}</button><button class="btn tiny soft" data-edit="1" data-type="note" data-id="${n.id}">Edit</button><button class="btn tiny danger" data-delete="1" data-type="note" data-id="${n.id}">Delete</button></div></article>`).join(''):`<div class="panel"><p class="muted">No notes found. Tap + Note to create one.</p></div>`; els.notesList.onclick=e=>{ const act=e.target.closest('[data-note-action]'), edit=e.target.closest('[data-edit]'), del=e.target.closest('[data-delete]'); if(act) toggleNoteAction(act.dataset.id,act.dataset.noteAction); if(edit) editEntry('note',edit.dataset.id); if(del) deleteEntry('note',del.dataset.id); }; }
  function toggleNoteAction(id,action){ const n=state.notes.find(x=>x.id===id); if(!n) return; if(action==='pin') n.pinned=!n.pinned; if(action==='favorite') n.favorite=!n.favorite; if(action==='archive') n.archived=!n.archived; n.updatedAt=new Date().toISOString(); log(`Note ${action}: ${n.title}`); save(`Note ${action} updated`); renderNotes(); autoSync(); }
  ...
  function renderActivityLog(){ if(!els.activityLogList) return; const items=(state.activity||[]).slice(0,60); els.activityLogList.innerHTML=items.length?items.map(a=>`<div class="activity-item"><strong>${esc(a.message||a.action||'Activity')}</strong><small>${new Date(a.at||Date.now()).toLocaleString('en-PK')}</small></div>`).join(''):'<p class="muted">No activity yet.</p>'; }
  function createMonthlyArchive(){ const mk=state.settings.selectedMonth; const expenses=monthFilter(state.expenses); const incomes=monthFilter(state.incomes); const exp=sum(expenses), inc=sum(incomes); const topExpense=expenses.slice().sort((a,b)=>b.amount-a.amount)[0]; const categoryTotals={}; expenses.forEach(e=>(e.categoryIds||[]).forEach(id=>categoryTotals[id]=(categoryTotals[id]||0)+Number(e.amount||0))); const personTotals={}; expenses.forEach(e=>(e.peopleIds||[]).forEach(id=>personTotals[id]=(personTotals[id]||0)+Number(e.amount||0))); const topCategory=Object.entries(categoryTotals).sort((a,b)=>b[1]-a[1])[0]; const topPerson=Object.entries(personTotals).sort((a,b)=>b[1]-a[1])[0]; const archive={id:'archive-'+mk,month:mk,label:fmtMonth(mk),income:inc,expense:exp,savings:inc-exp,entryCount:expenses.length+incomes.length,topExpense:topExpense?{title:topExpense.title,amount:topExpense.amount}:null,topCategory:topCategory?{name:nameById(state.categories,topCategory[0]),amount:topCategory[1]}:null,topPerson:topPerson?{name:nameById(state.people,topPerson[0]),amount:topPerson[1]}:null,createdAt:new Date().toISOString()}; state.monthArchives=(state.monthArchives||[]).filter(x=>x.id!==archive.id); state.monthArchives.unshift(archive); log(`Monthly archive created: ${archive.label}`); save(`Monthly archive created for ${archive.label}`); renderArchives(); toast('Monthly archive created.'); autoSync(); }
  function renderArchives(){ if(!els.archiveList) return; const list=(state.monthArchives||[]).slice(0,24); els.archiveList.innerHTML=list.length?list.map(a=>`<div class="archive-card"><div><strong>${esc(a.label)}</strong><small class="muted">${a.entryCount} entries · created ${new Date(a.createdAt).toLocaleDateString('en-PK')}</small></div><div class="archive-metrics"><span>Income ${money(a.income)}</span><span>Expense ${money(a.expense)}</span><span>Savings ${money(a.savings)}</span>${a.topExpense?`<span>Largest: ${esc(a.topExpense.title)} ${money(a.topExpense.amount)}</span>`:''}${a.topCategory?`<span>Top category: ${esc(a.topCategory.name)} ${money(a.topCategory.amount)}</span>`:''}${a.topPerson?`<span>Top person: ${esc(a.topPerson.name)} ${money(a.topPerson.amount)}</span>`:''}</div></div>`).join(''):'<p class="muted">No monthly archives yet.</p>'; }
  ...
  async function syncNow(show=true){ if(!state.settings.syncUrl) return toast('Add Google Apps Script URL first.'); if(!navigator.onLine) return toast('Offline. Saved locally and will sync later.'); try{ const cleanState = normalize(JSON.parse(JSON.stringify(state))); cleanState.meta.syncingDevice = deviceLabel(); const res=await fetch(state.settings.syncUrl,{method:'POST',headers:{'Content-Type':'text/plain;charset=utf-8'},body:JSON.stringify({action:'saveState',state:cleanState})}); const json=await res.json(); if(!json.ok) throw new Error(json.error||'Sync failed'); state.meta.lastSyncAt=new Date().toISOString(); state.meta.lastRemoteUpdatedAt=json.savedAt||state.meta.lastSyncAt; state.meta.syncPending=false; state.meta.syncQueue=[]; const c=json.counts||{}; state.meta.syncMessage=`Synced to Google Sheet. Expenses: ${c.expenses||0}, Income: ${c.incomes||0}, Notes: ${c.notes||0}.`; log(`Synced to Google Sheet (${c.expenses||0} expenses, ${c.incomes||0} income, ${c.notes||0} notes)`); localStorage.setItem(STORAGE_KEY, JSON.stringify(state)); mirrorToOfflineDB(); updateSyncStatus(); renderSettings(); renderActivityLog(); if(show) toast(state.meta.syncMessage); }catch(e){ state.meta.syncMessage='Sync failed. Data is still saved locally.'; log('Sync failed. Local data is safe.'); localStorage.setItem(STORAGE_KEY, JSON.stringify(state)); mirrorToOfflineDB(); updateSyncStatus(); renderSettings(); renderActivityLog(); if(show) toast('Sync failed. Local data is safe.'); console.error(e); } }
  async function importSheet(){ if(!state.settings.syncUrl) return toast('Add Google Apps Script URL first.'); if(!navigator.onLine) return toast('Internet is required to import from Sheet.'); const localChanges=(state.meta.syncQueue||[]).length; const warning=localChanges?`You have ${localChanges} pending local changes. Importing will replace this device's local data. Export JSON first if unsure. Continue?`:'Import Sheet will replace local data on this device. Continue?'; if(!confirm(warning)) return; try{ const res=await fetch(state.settings.syncUrl+'?action=getState'); const json=await res.json(); if(!json.ok || !json.state) throw new Error(json.error||'No state found'); const keepUrl=state.settings.syncUrl; const previousActivity=state.activity||[]; state=normalize(json.state); state.settings.syncUrl=state.settings.syncUrl||keepUrl; state.activity=[{id:uid('act'),message:'Imported data from Google Sheet',at:new Date().toISOString()},...previousActivity,...(state.activity||[])].slice(0,220); state.meta.lastImportAt=new Date().toISOString(); localStorage.setItem(STORAGE_KEY,JSON.stringify(state)); mirrorToOfflineDB(); renderAll(); toast('Imported from Google Sheet.'); }catch(e){ toast('Import failed.'); console.error(e); } }
  function exportJson(){ log('Exported full JSON backup'); save('Backup exported'); download(`family-expense-backup-${Date.now()}.json`, JSON.stringify(state,null,2),'application/json'); renderActivityLog(); }
  function exportCsv(){ log('Exported entries CSV'); save('Entries CSV exported'); const rows=[['type','date','title','description','amount','people','categories','source','account','necessary','status','tag']]; allEntries().forEach(x=>rows.push([x.type,x.date,x.title,x.description,x.amount||'',(x.peopleIds||[]).map(id=>nameById(state.people,id)).join('|'),(x.categoryIds||[]).map(id=>nameById(state.categories,id)).join('|'),x.source||'',nameById(state.accounts,x.accountId)||'',x.necessary===undefined?'':x.necessary,x.status||'',x.tag||''])); download(`family-expense-entries-${Date.now()}.csv`, rows.map(r=>r.map(v=>`"${String(v??'').replace(/"/g,'""')}"`).join(',')).join('\n'),'text/csv'); }
  function importBackup(e){ const f=e.target.files[0]; if(!f) return; const r=new FileReader(); r.onload=()=>{ try{ const next=normalize(JSON.parse(r.result)); const keepUrl=state.settings.syncUrl; state=next; state.settings.syncUrl=state.settings.syncUrl||keepUrl; localStorage.setItem(STORAGE_KEY,JSON.stringify(state)); mirrorToOfflineDB(); log('Imported JSON backup'); save('Backup imported'); renderAll(); toast('Backup imported.'); }catch(err){ toast('Invalid backup file.'); } }; r.readAsText(f); e.target.value=''; }
  ...
  function deviceLabel(){ const ua=navigator.userAgent||''; if(/Android|iPhone|iPad/i.test(ua)) return 'Mobile'; return 'Laptop/Desktop'; }

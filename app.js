(() => {
  ...
  async function syncNow(show=true){ if(!state.settings.syncUrl) return toast('Add Google Apps Script URL first.'); if(!navigator.onLine) return toast('Offline. Saved locally and will sync later.'); try{ const cleanState = normalize(JSON.parse(JSON.stringify(state))); const res=await fetch(state.settings.syncUrl,{method:'POST',headers:{'Content-Type':'text/plain;charset=utf-8'},body:JSON.stringify({action:'saveState',state:cleanState})}); const json=await res.json(); if(!json.ok) throw new Error(json.error||'Sync failed'); state.meta.lastSyncAt=new Date().toISOString(); state.meta.syncPending=false; state.meta.syncQueue=[]; const c=json.counts||{}; state.meta.syncMessage=`Synced to Google Sheet. Expenses: ${c.expenses||0}, Income: ${c.incomes||0}, Notes: ${c.notes||0}.`; localStorage.setItem(STORAGE_KEY, JSON.stringify(state)); mirrorToOfflineDB(); updateSyncStatus(); renderSettings(); if(show) toast(state.meta.syncMessage); }catch(e){ state.meta.syncMessage='Sync failed. Data is still saved locally.'; localStorage.setItem(STORAGE_KEY, JSON.stringify(state)); mirrorToOfflineDB(); updateSyncStatus(); renderSettings(); if(show) toast('Sync failed. Local data is safe.'); console.error(e); } }
